...
def training_loop(
    ...
    if rank == 0:
        ...
        try:
            import wandb
            if rank == 0:
                wandb.init(project="Transitional-cGAN", name=f"{run_dir.split('/')[-1]}", dir=run_dir)
            stats_wandb = True
        except ImportError as err:
    ...
    old_nimg = 0    # Added by the authors
    while True:
        ...
        for phase, phase_gen_z, phase_gen_c in zip(phases, all_gen_z, all_gen_c):
            ...
            for round_idx, (real_img, real_c, gen_z, gen_c) in enumerate(zip(phase_real_img, phase_real_c, phase_gen_z, phase_gen_c)):
                ...
                gain = phase.interval
                loss.accumulate_gradients(phase=phase.name, real_img=real_img, real_c=real_c, gen_z=gen_z, gen_c=gen_c, sync=sync, gain=gain, num_gpus=num_gpus)
        ...
        if training_set_kwargs.use_labels:
        ...
        if (rank == 0) and (image_snapshot_ticks is not None) and (done or cur_tick % image_snapshot_ticks == 0):
            images = torch.cat([G_ema(z=z, c=c, noise_mode='const').cpu() for z, c in zip(grid_z, grid_c)]).numpy()
            image_snapshot = save_image_grid(images, os.path.join(run_dir, f'fakes{cur_nimg//1000:06d}.png'), drange=[-1,1], grid_size=grid_size)    # Modified by the authors
            if stats_wandb:  # Added by the authors
                wandb.log({'gen_img': [wandb.Image(image_snapshot, caption="Generated Images")]}, step=cur_nimg//1000)  # Added by the authors
